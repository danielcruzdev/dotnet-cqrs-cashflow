...
public sealed class ConsultaExceptionHandler : IExceptionHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var (status, titulo, codigo, detalhe) = exception switch
        {
            ConsultaInvalidaException e => (
                StatusCodes.Status400BadRequest, "Consulta inválida", e.Codigo, e.Message),

            // Circuito aberto: o banco está degradado e falhar rápido preserva o
            // orçamento de erro em vez de empilhar requisições. O detalhe é texto
            // próprio: a mensagem do Polly descreve a implementação interna.
            BrokenCircuitException => (
                StatusCodes.Status503ServiceUnavailable,
                "Serviço temporariamente indisponível",
                "consolidado.circuito_aberto",
                "A consulta está suspensa por degradação do armazenamento. Tente de novo em alguns segundos."),

            TimeoutRejectedException => (
                StatusCodes.Status504GatewayTimeout,
                "Tempo de consulta excedido",
                "consolidado.timeout",
                "A consulta excedeu o tempo limite. Tente de novo em alguns segundos."),
